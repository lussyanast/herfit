...
class WaitingTransactions extends BaseWidget
{
    ...
    {
        $pk = (new Transaksi)->getKeyName(); // <= aman kalau PK bukan "id"
        ...
        return $table
            ->query(
                Transaksi::query()
                    ...
                    ->orderByDesc($pk)          // tie-breaker aman
            )
            ->columns([
                Tables\Columns\TextColumn::make('kode_transaksi')
                    ->label('Kode Transaksi')->searchable()->sortable()->copyable()->toggleable(),

                Tables\Columns\TextColumn::make('produk.kode_produk')
                    ->label('Kode Produk')->searchable()->toggleable(),
                ...
                Tables\Columns\TextColumn::make('pengguna.nama_lengkap')
                    ->label('Nama Pengguna')->sortable()->searchable(),

                Tables\Columns\TextColumn::make('pengguna.email')
                    ->label('Email Pengguna')->searchable()->copyable()->toggleable(),

                Tables\Columns\TextColumn::make('bukti_pembayaran')
                    ->label('Bukti Bayar')->html()
                    ...
                    ->action(
                        Action::make('lihat_bukti')->label('Lihat Bukti')->icon('heroicon-o-eye')
                ...
                Tables\Columns\BadgeColumn::make('status_transaksi')
                    ->label('Status')
                    ->colors(['gray' => 'waiting', 'success' => 'approved', 'danger' => 'rejected'])
                    ->sortable(),

                Tables\Columns\TextColumn::make('tanggal_mulai')
                    ->label('Mulai')->date('d M Y')->sortable()->toggleable(),

                Tables\Columns\TextColumn::make('tanggal_selesai')
                    ->label('Selesai')->date('d M Y')->sortable()->toggleable(),
            ...
            ->actions([
                Action::make('approve')->label('Setujui')->color('success')->icon('heroicon-o-check')
                    ...
                    ->action(function (Transaksi $record) {
                        ...
                        Notification::make()->title('Transaksi Disetujui')
                            ->body('Status transaksi berhasil diubah menjadi approved.')
                            ->success()->send();
                    }),

                Action::make('reject')->label('Tolak')->color('danger')->icon('heroicon-o-x-circle')
                    ->requiresConfirmation()
                    ->visible(fn(Transaksi $record) => $record->status_transaksi === 'waiting')
                    ->action(function (Transaksi $record) {
                        app(TransaksiFsm::class)->handle($record, 'reject');
                        Notification::make()->title('Transaksi Ditolak')
